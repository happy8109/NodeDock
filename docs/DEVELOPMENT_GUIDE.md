...
## 6. 后续扩展构思 (Roadmap)
*   **进程守护 (Auto Restart)**：监测 Node 进程状态，若发生异常退出且配置了“自动重启”，则尝试重新拉起应用。
*   **资源占用监控**：在主界面实时显示各 Node 进程的 CPU 与内存占用情况。
*   **自定义环境变量**：支持为每个应用单独配置额外的环境变量（如 `PORT`, `NODE_ENV` 等）。
*   **开机自启动**：添加注册表项实现 Windows 登录自动启动。
*   **分组管理**：支持将不同的 Node 程序按业务类型进行分组。
